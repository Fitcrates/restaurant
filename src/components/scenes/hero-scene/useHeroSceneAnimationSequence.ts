...
type UseHeroSceneAnimationSequenceParams = {
...
const SEQUENCE_FRAME_COUNT = 40;
const EXTRACTION_MAX_WIDTH = 960;

export default function useHeroSceneAnimationSequence({
  sceneRef,
  loopVideoARef,
  loopVideoBRef,
  loopStackRef,
  ...
  vignetteRef,
  smokeRef,
}: UseHeroSceneAnimationSequenceParams) {
  ...
  useLayoutEffect(() => {
    ...
    const featureVideo = featureVideoRef.current;
    if (!scene || !canvas || !featureVideo) return;
    ...
    const frameResponse = isTouchDevice ? 11 : 16;
    ...
    const drawBitmap = (bitmap: ImageBitmap) => {
      ...
      const sourceWidth = bitmap.width || canvas.width;
      const sourceHeight = bitmap.height || canvas.height;
    ...
    const drawFrameByIndex = (index: number) => {
      ...
      if (!bitmap || drawnFrameIndexRef.current === index) return;
      drawBitmap(bitmap);
    ...
    const waitForSeek = (video: HTMLVideoElement) =>
      new Promise<void>((resolve, reject) => {
        ...
        const onError = () => {
          cleanup();
          reject(new Error('Failed to seek video for sequence extraction'));
        };
        const cleanup = () => {
    ...
    const extractSequenceFrames = async () => {
      try {
        if (sequenceReadyRef.current || sequenceFailedRef.current) return;
        if (featureVideo.readyState < 2) return;

        const duration = featureVideo.duration && isFinite(featureVideo.duration) ? featureVideo.duration : 4;
        const safeEnd = Math.max(duration - 0.02, 0);
        const tempCanvas = document.createElement('canvas');
        const sourceWidth = featureVideo.videoWidth || 1920;
        const sourceHeight = featureVideo.videoHeight || 1080;
        const scaledWidth = Math.min(sourceWidth, EXTRACTION_MAX_WIDTH);
        const scaledHeight = Math.round((scaledWidth / sourceWidth) * sourceHeight);
        ...
        for (let i = 0; i < SEQUENCE_FRAME_COUNT; i += 1) {
          const progress = i / (SEQUENCE_FRAME_COUNT - 1);
          const targetTime = safeEnd * progress;
          featureVideo.currentTime = targetTime;
          await waitForSeek(featureVideo);
          tempCtx.drawImage(featureVideo, 0, 0, scaledWidth, scaledHeight);
          const bitmap = await createImageBitmap(tempCanvas);
          bitmapsRef.current.push(bitmap);
        }

        sequenceReadyRef.current = bitmapsRef.current.length === SEQUENCE_FRAME_COUNT;
        featureVideo.currentTime = 0.001;
    ...
    const gsapCtx = gsap.context(() => {
      ...
      const tick = (now: number) => {
        ...
        renderedProgressRef.current +=
          (targetProgressRef.current - renderedProgressRef.current) * getLerpFactor(progressResponse, deltaSeconds);
        ...
        if (sequenceReadyRef.current && phase !== 'intro' && phase !== 'crossfade') {
          ...
          const targetFrame = flipProgress * (SEQUENCE_FRAME_COUNT - 1);

          renderedFrameRef.current +=
            (targetFrame - renderedFrameRef.current) * getLerpFactor(frameResponse, deltaSeconds);

          const frameIndex = Math.max(0, Math.min(Math.round(renderedFrameRef.current), SEQUENCE_FRAME_COUNT - 1));
